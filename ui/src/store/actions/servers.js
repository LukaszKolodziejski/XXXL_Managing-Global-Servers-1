...
// Redux-Thunk
export const clickDropdown = (id, newStatus, data) => (dispatch) => {
  dispatch(clickDropdownStart());
  axios
    .put(`/${id}/${newStatus}`)
    .then((res) => {
      const { servers, reboot, searchServers, searchQuery } = { ...data };
      const { status } = res.data;
      if (!searchQuery) {
        for (const key in servers) {
          servers[key].id === id && (servers[key].status = status);
        }
      } else {
        for (const key in searchServers) {
          searchServers[key].id === id && (searchServers[key].status = status);
        }
      }
      dispatch(clickDropdownSuccess(servers, searchServers, !reboot));
    })
    .catch((err) => dispatch(clickDropdownFail(err.message)));
};
